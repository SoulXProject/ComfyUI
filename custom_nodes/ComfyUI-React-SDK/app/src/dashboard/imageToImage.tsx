...
const ImageToImage: React.FC<DashboardProps> = () => {
  const { advancedImagePrompt, uploadImage } = useComfy();
        ...
        <Stack direction="row" spacing={2} style={{ width: '100%' }}>
          <Box flex="1" style={{ padding: '20px' }}>
            <Stack direction="column" spacing={6} style={{marginTop: '5vh'}}>
              ...
              <TextField label="Positive Prompt" multiline rows={4} value={positivePrompt}
                         onChange={(e) => setPositivePrompt(e.target.value)} fullWidth/>
              <TextField label="Negative Prompt" multiline rows={4} value={negativePrompt}
                         onChange={(e) => setNegativePrompt(e.target.value)} fullWidth/>
