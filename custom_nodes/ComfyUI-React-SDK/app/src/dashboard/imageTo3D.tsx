...
const ImageTo3D: React.FC<DashboardProps> = () => {
  const { imageTo3dPrompt, uploadImage } = useComfy();
  ...
  useEffect(() => {
    Subscribe('dashboard', (event) => {
      ...
      if (message.type === WS_MESSAGE_TYPE_EXECUTED && message.data?.output?.mesh && message.data?.output?.mesh[0]?.type === "output") {
        setRand(Math.random());
        setOutputImage(message.data.output.mesh[0].filename);
        setInProgress(false);
        setProgress(0);
  ...
  const generate = () => {
    ...
    imageTo3dPrompt(inputImage).then((res: { prompt_id: any; }) => {
      if (res.prompt_id) {
